...
inputFile = 'input.txt'
encodedFile = 'encodedTxt.huff'
decodedFile = 'decodedTxt.txt'

# ENCODING FILE
with open(inputFile, 'rb') as in_file, open(encodedFile, "wb") as out_file:
    ...
    print(str("Original Length = " + str(len(fileByteArr) * 8)))
    print(str("Encoded Length Without redundant bits = " + str(encodedStringLen)))
    print(str("Encoded Length = " + str(len(outStr))))

    # Writing the bytes to the output file.
    out_file.write(outByteArr)
